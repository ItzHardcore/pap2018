...
<?php
  ...
  $q = mysql_query("SELECT COUNT(*) FROM recuperacao WHERE utilizador = '$email' AND confirmacao = '$hash'");
 
  if( mysql_result($q, 0, 0) == "1" ){
    // os dados estão corretos: eliminar o pedido e permitir alterar a password
    mysql_query("DELETE FROM recuperacao WHERE utilizador = '$email' AND confirmacao = '$hash'");
